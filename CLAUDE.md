...
- **Cron `/api/cron/cleanup`**: JEDAN dnevni Vercel cron (`vercel.json`, `0 7 * * *` = 07:00 UTC ≈ 09:00/08:00 po srpskom). Vercel Hobby dozvoljava 1 cron → sve je u njemu: (1) GDPR cleanup (rate-limit/analytics/booking PII), (2) **auto check-out** (`autoCheckoutDueBookings` — `confirmed`/`checked_in` sa prošlom odjavom → `checked_out`; preskače `no_show`/`cancelled`), (3) scheduled mejlovi (`processScheduledEmails`: check-in uputstva dan pre, podsetnik 3 dana pre, recenzija dan posle). Zaštita: `CRON_SECRET`.
