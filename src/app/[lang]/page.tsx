...
import { getContentText, getPublishedSectionContent } from '@/lib/content/public-content'
...
export default async function HomePage({ params: paramsInput }: PageProps) {
  ...
  const homeContent = await getPublishedSectionContent('home', locale)
  ...
  const faqItems = [
    { question: homeText('faq.q1'), answer: homeText('faq.a1') },
    { question: homeText('faq.q2'), answer: homeText('faq.a2') },
    { question: homeText('faq.q4'), answer: homeText('faq.a4') },
    { question: homeText('faq.q5'), answer: homeText('faq.a5') },
    { question: homeText('faq.q7'), answer: homeText('faq.a7') },
    { question: homeText('faq.q8'), answer: homeText('faq.a8') },
    { question: homeText('faq.q9'), answer: homeText('faq.a9') },
    { question: homeText('faq.q10'), answer: homeText('faq.a10') }
  ]
        ...
        <div className="container relative z-20 text-center text-white max-w-3xl mx-auto px-8">
          <div className="stagger-fade-in flex flex-col items-center">
            ...
            <div className="flex flex-wrap gap-4 justify-center items-center">
              ...
              >
                <span>{homeText('hero.whatsAppLabel', 'WhatsApp')}</span>
              ...
              >
                <span>{homeText('hero.viberLabel', 'Viber')}</span>
      ...
      <section className="border-y border-white/20 py-6">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              { name: homeText('trust.secure'), icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg> },
              { name: homeText('trust.flexible'), icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> },
              { name: homeText('trust.prime'), icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg> },
              { name: homeText('trust.bestPrice'), icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138z" /></svg> },
            ].map((item) => (
          ...
          <div className="flex flex-col md:flex-row gap-16 items-center">
            <div className="flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {[
                  ...
                  { name: homeText('features.kitchen'), icon: '🍳' },
                  { name: homeText('features.tv'), icon: '📺' },
                  { name: homeText('features.linens'), icon: '🛏️' },
                  { name: homeText('features.lakeAccess'), icon: '🏖️' },
                  { name: homeText('features.bbq'), icon: '🔥' },
                ].map((item) => (
            ...
            <div className="flex-1 relative">
              ...
              <div className="absolute -bottom-10 -left-10 bg-primary/90 p-6 rounded-2xl shadow-xl max-w-sm hidden lg:block text-primary-foreground transform -rotate-3 border-4 border-white/50 backdrop-blur-md">
                <p className="font-bold italic text-lg mb-2">&quot;{homeText('testimonials.sampleQuote')}&quot;</p>
                <div className="flex items-center gap-2">
                  <div className="w-10 h-10 rounded-full bg-primary-foreground/20 flex items-center justify-center">{homeText('testimonials.sampleInitials')}</div>
                  <div>
                    <p className="font-bold">{homeText('testimonials.sampleName')}</p>
                    <p className="text-xs opacity-80">{homeText('testimonials.guestFrom')} {homeText('testimonials.sampleLocation')}</p>
                  </div>
                </div>
              </div>
            </div>
                ...
                <CardContent className="p-6">
                  <div className="flex justify-between items-center mb-6">
                    <div>
                      ...
                      <span className="text-muted-foreground text-sm"> / {homeText('featured.perNight', apartmentText('perNight'))}</span>
                    </div>
                  ...
                  <Link href={`/${params.lang}/booking?apartment=${apt.slug || apt.id}`} className="w-full block">
                    <span className="w-full h-12 flex items-center justify-center bg-primary text-white hover:opacity-90 rounded-full font-bold text-base transition-all cursor-pointer">
                      {homeText('featured.checkAvailability')}
                    </span>
                  </Link>
                </CardContent>
              </Card>
        ...
        <div className="container mx-auto px-4 md:px-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            <div className="space-y-6">
              {[
                { q: homeText('faq.q1'), a: homeText('faq.a1') },
                { q: homeText('faq.q2'), a: homeText('faq.a2') },
                { q: homeText('faq.q4'), a: homeText('faq.a4') },
                { q: homeText('faq.q5'), a: homeText('faq.a5') },
              ].map((faq, i) => (
            ...
            <div className="space-y-6">
              {[
                { q: homeText('faq.q7'), a: homeText('faq.a7') },
                { q: homeText('faq.q8'), a: homeText('faq.a8') },
                { q: homeText('faq.q9'), a: homeText('faq.a9') },
                { q: homeText('faq.q10'), a: homeText('faq.a10') },
              ].map((faq, i) => (
                <div key={i} className="bg-white/85 p-6 rounded-2xl border border-white/40 backdrop-blur-md">
                  <h3 className="text-lg font-bold mb-2 text-foreground">{faq.q}</h3>
                  <p className="text-muted-foreground">{faq.a}</p>
                ...
                <div key={review.id} className="bg-white/85 p-8 rounded-2xl shadow-sm border border-white/40 backdrop-blur-md">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{homeText('testimonials.guestFrom')} {new Date(review.created_at).toLocaleDateString()}</p>
                    </div>
                  </div>
                </div>
              ))}
